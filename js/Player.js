...
export class Player {
    constructor({
        ...
        this.animat = [
            ...
            engine.createAnimatedSprite(engine.textureArr(frameNumbers[6], "PL" + fighterNumber.toString() + "_attack_right"), { x: position.x, y: position.y }, 2.5, 0.15, { x: 0, y: 0.5 }, false)
        ]
        ...
        this.attackBoxLeft = new Graphics().beginFill(0x000000, 0.7).drawRect(this._view.getChildAt(0).position.x - 200, this._view.getChildAt(0).position.y - 40, 210, 50).endFill();
        this.hitBox = new Graphics().beginFill(0xff0000, 0.7).drawRect(this._view.getChildAt(0).position.x - this.animat[0].width / 4, this._view.getChildAt(0).position.y - this.animat[0].height / 2, this.animat[0].width / 2, this.animat[0].height).endFill();
    ...
    attack() {
        ...
        if (this.lastKey === 'd' || this.lastKey === 'ArrowRight') {
            ...
            if (this.animat[6].anchor.x !== 0.5)
                this.animat[6].anchor.x = -1 * this.animat[6].scale.x;
        }
